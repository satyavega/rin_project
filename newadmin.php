...
<aside id="sidebar-multi-level-sidebar" class="fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
   <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
      <ul class="space-y-2 font-medium">
         <li>
            <a href="#product" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
               <svg aria-hidden="true" class="w-6 h-6 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M2 10a8 8 0 018-8v8h8a8 8 0 11-16 0z"></path><path d="M12 2.252A8.014 8.014 0 0117.748 8H12V2.252z"></path></svg>
               <span class="ml-3">Products</span>
            </a>
         </li>
         <li>
            <a href="newadminadd.php" class="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700">
...
<div class="p-4 sm:ml-64">
   <div class="p-4 border-2 border-gray-200 dark:border-gray-700">
        <div class="grid gap-4 mt-4 py-3.5 mb-4" id="product">
        <h2 class="mb-4 text-4xl font-bold tracking-tight text-gray md:text-4xl lg:text-6xl">Products Table</h2>
        <table class="min-w-full bg-white border border-gray-300">
    <thead>
        <tr>
        <th class="px-6 py-3 bg-gray-100 text-left text-sm font-semibold text-gray-600">Img Product</th>
        <th class="px-6 py-3 bg-gray-100 text-left text-sm font-semibold text-gray-600">Product Name</th>
        <th class="px-6 py-3 bg-gray-100 text-left text-sm font-semibold text-gray-600">Product Code</th>
        <th class="px-6 py-3 bg-gray-100 text-left text-sm font-semibold text-gray-600">Product Description</th>
        <th class="px-6 py-3 bg-gray-100 text-left text-sm font-semibold text-gray-600">Qty</th>
        <th class="px-6 py-3 bg-gray-100 text-left text-sm font-semibold text-gray-600">Action</th>
        </tr>
    </thead>
    <tbody>
        <?php
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            echo '<tr>';
            echo '<td class="px-6 py-4"><img src="' .$row['product_img_name'] . '" alt="Product Image" class="w-12 h-12 object-cover rounded"></td>';
            echo '<td class="px-6 py-4">' . $row['product_name'] . '</td>';
            echo '<td class="px-6 py-4">' . $row['product_code'] . '</td>';
            echo '<td class="px-6 py-4">' . $row['product_desc'] . '</td>';
            echo '<td class="px-6 py-4">' . $row['qty'] . '</td>';
            echo '<td class="px-6 py-4">';
            echo '<div class="flex">';
            echo '<a href="editadmin.php?id='  .$row['id'] . ' " class="bg-blue-500 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded">Update</button>';
            echo '<a href="delete_product.php?id=' . $row['id'] . '" class="ml-2 bg-red-500 hover:bg-red-700 text-white font-semibold px-4 py-2 rounded">Delete</a>';
            echo '</div>';
            echo '</td>';
            echo '</tr>';
        }
